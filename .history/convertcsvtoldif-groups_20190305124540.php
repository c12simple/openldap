...
// Create ou:
    ...
    while ((($data = fgetcsv($handle, 1000, ",")) !== FALSE) && ($row <= $limit)){
        ...
        {
            echo "dn: uid=".$data[2].",ou=".$dn: uid=rmctaguec2,ou=controller,ou=staff,ou=people,dc=example,dc=org
            changetype: add
            gidNumber: 0
            ...
            uidNumber: 6435
            uid: rmctaguec2
            homeDirectory: /home/rmctaguec2
            sn: Mélia
            cn: Mélia
            mail: rmctaguec2@vpydio.fr
            employeeType: controller
            displayName: Name Mélia
            memberOf: cn=Subsurface,ou=groups,dc=example,dc=org
            userPassword:: UEBzc3cwcmQ=[$data[4]].",ou=staff,ou=people,dc=example,dc=org\n";
        }else{
        ...
        {
            echo "memberOf: cn=".$departMent[$data[5]].",ou=groups,dc=example,dc=org\n";
        }
