...
// Create ou:
    ...
    while ((($data = fgetcsv($handle, 1000, ",")) !== FALSE) && ($row <= $limit)){
        ...
        {
            $userDn = "dn: uid=".$data[2].",ou=".$departMent[$data[5]].",ou=people,dc=example,dc=org\n";
        }else{
            $userDn = "dn: uid=".$data[2].",ou=staff,ou=people,dc=example,dc=org\n";
        }
        echo $$userDn;
        echo "changetype: add\n";
        echo "gidNumber: 0\n";
        ...
        if ( isset($userTypeGroups[$data[4]]) )
        {
            echo "cn=".$userTypeGroups[$data[4]].",ou=groups,dc=example,dc=org\n";
