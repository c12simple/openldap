...
// Create ou:
    ...
    while ((($data = fgetcsv($handle, 1000, ",")) !== FALSE) && ($row <= $limit)){
        $num = count($data);
        echo "# New entry \n";
        if ( isset($departMent[$data[4]]))
        {
            echo "dn: uid=".$data[5].",ou=".$departMent[$data[4]].",ou=staff,ou=people,dc=example,dc=org\n";
        }else{
            echo "dn: uid=".$data[5].",ou=staff,ou=people,dc=example,dc=org\n";
        }
        echo "changetype: add\n";
        echo "gidNumber: 0\n";
